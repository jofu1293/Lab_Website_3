...
app.post('/login', (req, res, next) => {
    ...
    .then((data) => {
        if(!data){
            req.flash('error_msg', 'Email not registered to an account.');
          ...
          db.tx(tkn => {
              return tkn.one('SELECT * FROM users WHERE \'' + forgot + '\' = email;')
                .then(data =>{
                    ...
                    link2 ="http://"+req.get('host')+"/reset/"+token;
                    var mailOptions2 = {
                        to: data.email,
...
app.get('/reset/:token', function(req, res){
    ...
    .then(data=>{
        if(!data){
            ...
            res.render('pages/reset', req.body);
            console.log('Reset Link Verified');
    ...
    if (errors.length > 0) {
        ...
        async.waterfall([
            function(done) {
                ...
                db.tx(reset => {
                    return reset.oneOrNone('SELECT * FROM users WHERE reset_token <> $1 AND CAST(token_expires AS BIGINT)> $2', ['0', Date.now()])
                        .then(data=>{
                            if(!data){
                                req.flash('error', 'Password reset token is invalid or has expired.');
                                return res.redirect('/forgot');
